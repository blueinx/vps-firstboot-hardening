...
sshd_t_value() {
  local key="$1"
  "${SSHD_BIN}" -T | awk -v k="${key}" '$1==k {value=$2} END {if (value != "") print value}'
}

normalize_permitrootlogin_value() {
  case "$1" in
    without-password) printf '%s\n' "prohibit-password" ;;
    *) printf '%s\n' "$1" ;;
  esac
}
...
assert_ssh_state() {
  ...
  if [[ "${run_key}" -eq 1 ]]; then
    ...
    if [[ "$(normalize_permitrootlogin_value "${v}")" != "$(normalize_permitrootlogin_value "${root_login}")" ]]; then
      die "断言失败：permitrootlogin=${v}，期望 ${root_login}"
    fi
  fi
